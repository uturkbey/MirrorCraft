...
public class SimulationPanel extends JPanel {
   // ArrayList<Selectable> objects = new ArrayList<Selectable>
   //Since the selectable is not done yet we cannot make it finished but each object will also extend selectable so when we use this array it will do the desired job by using polymorphism
...
//      add(toolSectionCreaterSouth , BorderLayout.PAGE_END);
   ...
   public void paintComponent(Graphics g) 
   {
      ...
      // clear lightsOfObjects
      lightsOfObjects.clear();
      
      // no objects situation
      ...
      // lighhts transfer lightsOfObjects
      for ( int i = 0; i < lights.size(); i++ )
      {
         lightsOfObjects.add( lights.get(i) );
      }
      
      // use reflectReturner for lightsOfObjects
      for ( int i = 0; i < lightsOfObjects.size(); i++ )
      {
         if ( objectContainer.getCloser( lightsOfObjects.get(i) ) != null )
         {
            lightsOfObjects.add( objectContainer.getCloser( lightsOfObjects.get(i) ).reflectReturner( lightsOfObjects.get(i)) );
         }
      }
   ...
   private class simulationClickListener extends MouseAdapter {
      ...
      public void mousePressed(MouseEvent event) {
         ...
         if (trialtoolPanel.getNumber() == 1 || trialtoolPanel.getNumber() == 2 || trialtoolPanel.getNumber() == 8) {
            if (isFirstClick) {
...
// for taking three points after and after
         if (trialtoolPanel.getNumber() == 3 || trialtoolPanel.getNumber() == 4) {
            if (cycler == 1) {
               xPosition = event.getX();
               yPosition = event.getY();
               cycler = 2;
               ...
               } else if (trialtoolPanel.getNumber() == 4) {
                  System.out.println(xThirdPosition + " " + yThirdPosition);
                  repaint();
                  //currentObject = new Lens( xPosition, yPosition , xSecondPosition, ySecondPosition, xThirdPosition, yThirdPosition);
                  // objectContainer.add(currentObject);
